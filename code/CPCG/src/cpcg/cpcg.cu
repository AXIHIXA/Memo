...
int test(const char * matrixFile)
{
    ...
    mtx_header(matrixFile, &num_lines, &num_rows, &num_cols, &nnz, &is_symmetric);
    printf("\nmatrix name: %s\n"
           "num. rows:   %d\n"
           "num. cols:   %d\n"
           "nnz:         %d\n"
           "structure:   %s\n\n",
           matrixFile, num_rows, num_cols, nnz,
    ...
    mtx_parsing(matrixFile, num_lines, num_rows, nnz, h_A_rows,
...
void init_py_module(pybind11::module_ & m)
{
    ...
    m.def("test", test, py::arg("matrix_file"));
}
