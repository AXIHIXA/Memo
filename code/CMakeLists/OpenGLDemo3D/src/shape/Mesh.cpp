...
Mesh::Mesh(Shader * pShader, std::vector<Vertex> vertices, const glm::mat4 & model) : Mesh(pShader, model)
{
...
Mesh::Mesh(Shader * pShader, const glm::mat4 & model) : Shape(pShader, model)
{
    glBindVertexArray(vao);
    glBindBuffer(GL_ARRAY_BUFFER, vbo);
