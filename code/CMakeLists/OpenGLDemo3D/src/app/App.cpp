...
void App::initializeObjectsToRender()
{
    ...
    pTetrahedron = std::make_unique<Tetrahedron>(pMeshShader.get(),
                                                 glm::translate(glm::mat4(1.0f), {-2.0f, 0.0f, 0.0f}));

    pTriangle = std::make_unique<Mesh>(
            pMeshShader.get(),
            ...
            glm::rotate(glm::translate(glm::mat4(1.0f), {2.0f, 0.0f, 0.0f}),
    ...
    pSphereShader = std::make_unique<Shader>("src/shader/sphere.vert.glsl",
                                             ...
                                             "src/shader/phong.frag.glsl");
    pSphere = std::make_unique<Sphere>(pSphereShader.get(),
...
void App::render()
{
    auto ft = static_cast<float>(timeElapsedSinceLastFrame);
    ...
    pMeshShader->setInt("displayMode", displayMode);
    pTetrahedron->render(ft);
    pTriangle->render(ft);
    ...
    pSphereShader->setInt("displayMode", displayMode);
    pSphere->render(ft);
}
