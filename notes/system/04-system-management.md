...
- Symbolic mode (`MODE`)
  ...
  - `perms`:
    - `perms` is either:
      - zero or more letters from the set `rwxXst`, or
      - a single letter from the set `ugo`.
    - OPTION ONE: `rwxXst` select file mode bits for the affected users:
      - read (r),
      - write (w),
      - execute (or search for directories) (x).
    - OPTION TWO: `ugo` refers to `-+=` the permissions:
      - granted to the user who owns the file (u),
