...
### 🌱 [定义](https://en.cppreference.com/w/cpp/language/definition)（Definitions）
...
- 所有的 *声明* 都是定义，除以下情况**例外**
    ...
    ```
    - 在类的定义中的`非 inline（ C++17 起）` *静态数据成员* 的声明 
    ```
    struct S 
    {
        int n;                        // 定义 S::n
    ...
    int S::i;                         // 定义 S::i
    ```
    - （`C++17`起弃用）已经在类中用`constexpr`说明符定义过的 *静态数据成员* ，在 *命名空间作用域* 中的声明 
    ```
    struct S 
    {
