...
- [临时量生存期](https://en.cppreference.com/w/cpp/language/lifetime)：
    ...
    - 上一条有下列 *五条* **例外**
        1. *函数返回语句* 中绑定到 *引用类型返回值* 的 *临时量* 
            - 只存在到返回表达式的末尾。这种函数始终返回 *悬垂引用* （Dangling reference） 
            - 但函数返回`std::move(temp)`右值引用是可以的
                - 悬垂引用是因为临时量会被析构，底层数据跟着没了，而返回右值引用的话则只会析构一个寂寞，伤不着底层数据 => 13.6 
...
- 【终极分析】当一个 *函数返回右值引用* 时，究竟发生了什么
    - 背景
        - 某函数在其函数体内构造了一个`T`类型 *自动对象* `obj`，需要返回
        - 正常返回`T`类型的话，`return obj;`会用`obj`初始化一个`T`类型返回值对象`ret`，这会经历一次昂贵的 *拷贝初始化* 
        - 我们想直接 *复用* 已经构造好的`obj`的底层数据（资源）
    - 需求分析：为什么选择返回 *右值引用*
        1. 如何规避 *拷贝初始化* 返回值对象`ret`：返回 *引用* 即可，这样就是 *引用初始化* 了，可以做到复用资源
        2. 但这又产生了新的问题
            - *自动对象* 会在函数返回语句`return obj;`末尾被销毁
            - 也就是说我们这么搞，函数就会返回 *悬垂引用* （dangling reference），纯属作大死
        3. 解决办法就是返回 *右值引用* 
            - 注意，返回 *右值引用* 并不延续`obj`的生存期，也就是说`obj`依旧会在函数返回语句末被销毁
                - *函数返回语句中绑定到引用类型返回值的临时量的生存期不被延长，到返回语句末为止* 
            - 但通过 *右值引用* 和 *移动构造函数* ，我们会在`obj`被销毁之前解除它对自己的资源的控制，因此销毁`obj`并不会销毁它的底层数据
                - 这里先记住这句话，至于具体为什么后面会讲
        4. 但这又有了一个问题：
            - `obj`是变量，变量都是 *左值* ， *左值* 不能直接绑定到 *右值引用* 上，即使这个变量自己也是 *右值引用* 类型也不行
                - 搞不懂这句话的人都是把 *（值的）类型* （type）和 *值类别* （value category）这俩货给搞混了
                - 比如`T && a;`， *（值的类型）是右值引用* 说的是`T &&`， *（值类别）是左值* 说的是`a`，压根不是一回事儿
            - 解决办法就是`return std::move(obj);`
                - `std::move`的效果是，通过强制类型转换，让编译器强行把本来是`T`类型的`obj` *解释* 成`T &&`类型
                    - `T &&`的类型是 *右值引用* ，因此`T &&`类型的对象的 *值类别* 一定是 *右值*
                    - 注意这里的强制类型转换是在 *强行解释原有对象* ，并不是在 *临时生成一个右值引用类型的临时对象* 
            - 这样右值引用类型的返回值对象`ret`就可以绑定到`obj`上了
    - 具体实际发生了什么
        1. `return std::move(obj);`会用右值`obj`初始化`T &&`类型返回值对象`ret`，这会调用`T`类型的 *移动构造函数* 
            - 假设`T`类型的底层数据是由 *指针* 来指向的
            - 移动构造函数会先让`ret`的所有指针指向`obj`的底层数据，然后将`obj`的所有指针置为`nullptr`
            - 这样`obj`就失去了对这些底层数据的 *管辖权* ，而这个管辖权被`ret` *接管* 了
        2. 在表达式`return std::move(obj);`的末尾，`obj`会被析构。但析构函数`delete`掉的指针全部都是`nullptr`，所以实际上 *啥也没释放* 
            - 这样这部分底层数据就被`ret`安全地所有了
        3. 之后会有表达式`T && var = fun();`或者`var = fun();`用于接收`ret`
            - 对于前者，实际上是在用`ret` *引用初始化* `var`，这会调用`T`类型的 *移动构造函数* 
            - 对于后者，实际上是在用`ret`向`var` *赋值* ，这会调用`T`类型的 *移动赋值运算符* 
            - 不论是哪种，这步操作一样会把`ret`对底层数据的管辖权 *转移* 给`var`
        4. 在上面的表达式的末尾，`ret`会被释放。但`ret`同样已经没有指针指向底层数据了，所以这步释放一样是 *释放了个寂寞* 
        5. 到现在为止，`obj`的底层数据经过了两步 *转手* ，成功转移到了`var`名下
            - 相比返回`T`类型要经历的两步拷贝构造或者拷贝赋值，性能提高到不知哪里去了
- 右值引用和成员函数
