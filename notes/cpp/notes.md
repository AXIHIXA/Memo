...
- [临时量生存期](https://en.cppreference.com/w/cpp/language/lifetime)：
    - 一旦 *常左值引用* 或 *右值引用* 被绑定到 *临时量* 或其子对象，临时量的生存期就 *被延续* 以匹配 *该引用* 的生存期
        ...
        ```
        // 此函数将返回悬垂引用
        // 默认实参 临时量A() 绑定给了 常左值引用形参 a ，那么 A() 的生存期就到 函数返回 为止
        // 至于说 a 又被 绑定给了 第二引用 常引用返回值，那跟 A() 的生存期就半毛钱关系都没有了
    ...
    - 上一条有下列 *五条* **例外**
        1. *函数返回语句* 中绑定到 *引用类型返回值* 的 *临时量* 
            ...
            - 但函数返回`std::move(temp)`右值引用是可以的 => 13.6 
...
- 【终极分析】当一个 *函数返回右值引用* 时，究竟发生了什么
    - 背景
        - 某函数在其函数体内构造了一个`T`类型自动对象`obj`，需要返回
        - 正常返回`T`类型的话，`return obj;`会用`obj`初始化一个`T`类型返回值对象`ret`，这会经历一次昂贵的 *拷贝初始化* 
        - 我们想直接 *复用* 已经构造好的`obj`的底层数据（资源）
    - 需求分析
        1. 如何规避拷贝初始化返回值：返回 *引用* 即可，这样就是 *引用初始化* 了，可以做到复用资源
        2. 但这又产生了新的问题
            - 自动对象会在函数返回语句末被销毁
            - 也就是说我们这么搞，函数就会返回 *悬垂引用* ，纯属作大死
        3. 解决办法就是返回 *右值引用* 
