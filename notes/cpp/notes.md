...
### 🌱 [返回值优化](https://en.cppreference.com/w/cpp/language/copy_elision)（Return Value Optimization, `RVO`）

- [一处讲得很好的博客](https://shaharmike.com/cpp/rvo/#return-value-optimization)
- 我们用`struct S35`举栗子

#### 返回值优化

- 编译器被允许避免为返回值创造 *临时对象* ，即使绕过的用户代码有 *边缘效应* （side effects，例如输出等）
- 第一个栗子
```
S35 exampleRVO()
{
...
$ g++ -std=c++14 -fno-elide-constructors main.cpp && ./a.out
S35()
S35(S35 &&)
~S35()
S35(S35 &&)
...
~S35()
```
    1. 不加`-fno-elide-constructors`时，编译器没有调用用户代码，即使有 *边缘效应* （这个例子里就是输出）
        - 这是 *默认行为* ，也就是说实践中所有`C++`程序均采取`RVO`
    2. 1
