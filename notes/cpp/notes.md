...
```
- *临时量* 生存期：
    - 一旦 *引用* 被绑定到 *临时量* 或其子对象，临时量的生存期就 *被延续* 以匹配引用的生存期，但有下列**例外**
        1. *函数返回语句* 中绑定到 *引用类型返回值* 的 *临时量* 
            - 只存在到返回表达式的末尾。这种函数始终返回 *悬垂引用* （Dangling reference） 
            - *右值引用* 及 *常量左值引用* 所绑定对象的 *生存期会被延续* 到该引用的作用域结尾，因此返回`std::move(temp)`右值引用是可以的
...
- *显式默认* 和 *删除函数* 
    ...
    - 有些情况反而应当 *阻止* 拷贝或赋值，方法有
        - 对应控制成员定义为 *删除* 的函数
        - 对应控制成员 *声明但不定义* 为 *私有* 的函数（没有`= delete;`时的做法）
            - 声明但不定义 *成员函数* 是合法操作，除一个**例外** => 15.2.1
