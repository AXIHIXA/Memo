...
- 一些常识
    ...
    - 指针和引用
        ...
        - 指针的类型必须与绑定的对象的类型严格一致
            - 两个例外：常量指针指向非常量对象；基类指针指向派生类对象
        - 引用的类型必须与绑定的对象的类型严格一致
            - 两个例外：常量引用绑定到任何能转化为常量引用的对象（包括字面值）；基类引用绑定到派生类对象上
...
- 指针或引用的类型必须与其所引用的对象的**类型严格一致**，但有2个例外：
    - 指针：常量指针指向非常量对象；基类指针指向派生类对象
    - 引用：常量引用绑定到任何能转化为常量引用的对象（包括字面值）；基类引用绑定到派生类对象上

- **常量引用可以绑定在（其它类型的）右值上**。尤其，允许为一个常量引用绑定：
    - 非常量的对象
    - 字面值
    - 一般表达式
