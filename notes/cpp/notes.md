...
- 常见规则
    ...
    - 下标遍历型`for`循环的唯一指定写法，例如遍历`std::vector`：`for (std::vector::size_type i = 0; i != 10; ++i) {}`， *括号内声明* `i`（避免冲突）、`std::vector::size_type`（精确匹配类型，别啥玩意都整成`int`），`!=`（为了性能和一般至少都会实现`==`和`!=`，而`<`则不一定了）和`++i`（为了性能，严禁乱用`i++`）这四条你要细品
