...
__global__
void readColumnStoreColumn(float * __restrict__ dst, const float * __restrict__ src, int n)
{
...
__global__
void readRowStoreRow(float * __restrict__ dst, const float * __restrict__ src, int n)
{
    auto idx = static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x);
    auto idy = static_cast<int>(blockIdx.y * blockDim.y + threadIdx.y);
...
int main(int argc, char * argv[])
{
    ...
    // Acutal Tests
    readColumnStoreColumn<<<mGridDim, kBlockSize>>>(
            dOut.begin().base().get(),
            dIn.begin().base().get(),
            nn
    );
    ...
    readRowStoreRow<<<mGridDim, kBlockSize>>>(
            dOut.begin().base().get(),
            dIn.begin().base().get(),
            nn
    );
