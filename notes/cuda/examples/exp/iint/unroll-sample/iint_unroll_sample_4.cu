...
{
    auto idx = static_cast<int>(blockIdx.x * kBlockDim.x * kBlockDim.y * 4 + threadIdx.y * blockDim.x + threadIdx.x);

//    if (idx + 3 * kBlockSize >= sampleLen)
//    {
...
//        printf("%d\n%d\n%d\n%d\n", idx, idx + kBlockSize, idx + 2 * kBlockSize, idx + 3 * kBlockSize);
//        printf("%d,\n", idx);
...
int main(int argc, char * argv[])
{
    ...
    dim3 mGridDim {(numSamples + 4 * kBlockSize - 1) / (4 * kBlockSize), 1U, 1U};
