...
> 如果你认为这是你个人行为，自主安装的，放在`/usr/local/lib`
> 如果你觉得`jdk`对你来说是必不可少的运行库，放在`/lib`里

> 上面三句是最开始的想法。

> 其实我也想找出一个最佳实践，后来看了看`linux`的目录结构，
> 发现就算是同一个东西，系统自带和你手动安装，就不应该在同一个目录里。
> 同样是浏览器，系统自带的`firefox`就在`/usr/lib`里，
> 而后来通过软件包安装的`chrome`就在`/opt`里。
> 如果系统自带`java`，我觉得他会在`/lib`里或者`/usr/lib`，看它对`java`的定义是必需的库还是
